export const meta = {
  name: 'blackholes-graph-build',
  description: 'Coverage-driven sequential build of the LHC black-hole-safety argument graph from a deep-research report + its sources. Persona-diverse Sonnet authors run one at a time (each sees all prior work) so every isolatable claim gets extracted, placed, phrased, and connected without duplication.',
  phases: [{ title: 'Build' }],
}
...
function prompt(a, idx, total) {
  return `You are author "${a.id}" (#${idx + 1} of ${total}) building a COOPERATIVE argument graph for "Could a high-energy particle collider (the LHC) create a black hole that destroys Earth?". Working directory: ${V0} — cd there first. Dataset: --data blackholes-graph.
...
phase('Build')
log(`blackholes-graph build: ${authors.length} authors, sequential (each sees prior work)`)
const results = []
