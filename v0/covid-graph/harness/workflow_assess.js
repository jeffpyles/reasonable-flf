export const meta = {
  name: 'covid-graph-assess',
  description: 'Non-adversarial panel assessment of the fresh covid-graph: a persona-diverse 4-agent panel each independently rates every node AND every ground edge on Agreement (enforced blind Rating mode). Produces the dispersion/calibration/contested-crux signals that distinguish the assessed graph from the flat report.',
  phases: [{ title: 'Assess' }],
}
...
function prompt(a) {
  return `You are panelist "${a.id}" in a COOPERATIVE, non-adversarial BLIND assessment of the SARS-CoV-2-origins argument graph. Working directory: ${V0} — cd there first. Dataset: --data covid-graph.
...
Commands:
  python3 graph.py get-node <nid> --data covid-graph --json     # for a node, or the node an edge touches
  python3 graph.py rate --agent ${a.id} --target <target> --dim A --value <0.0-5.0> --bloc <bloc> --data covid-graph --json
...
phase('Assess')
log(`covid-graph assess: ${panel.length}-agent panel rating all nodes + edges on A, blind`)
