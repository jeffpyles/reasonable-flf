export const meta = {
  name: 'eggs-graph-build',
  description: 'Coverage-driven sequential build of a fresh egg-health argument graph from a deep-research report + its sources. Persona-diverse Sonnet authors run one at a time (each sees all prior work) so every isolatable claim gets extracted, placed, phrased, and connected without duplication. Backbone for the report-vs-graph comparison.',
  phases: [{ title: 'Build' }],
}
...
function prompt(a, idx, total) {
  return `You are author "${a.id}" (#${idx + 1} of ${total}) building a COOPERATIVE argument graph for "Are eggs bad for your health? (Does egg / dietary-cholesterol consumption meaningfully raise cardiovascular-disease and mortality risk for most people?)". Working directory: ${V0} — cd there first. Dataset: --data eggs-graph.
...
phase('Build')
log(`eggs-graph build: ${authors.length} authors, sequential (each sees prior work)`)
const results = []
