export const meta = {
  name: 'eggs-p10-bias-x-model',
  description: 'Balanced 2x2 INTERMEDIATE-bias x model run (transition-zone map): 28 agents (7 biased-haiku, 7 biased-sonnet, 7 neutral-haiku, 7 neutral-sonnet) each rate all 79 node claims blind (enforced Rating mode). De-confounds the disposition/capability axes entangled in eggs-p7, and asks whether camp-detection splits by disposition or by model. Offline analysis reuses the coldstart-lab scripts against eggs-p10, with both the p5 expert panel and the Fable+Opus panel anchors as references.',
  phases: [{ title: 'Rate' }],
}

const ROOT = '/home/user/reasonable'
const H = 'eggs-p10/harness'
...
This dataset is in ENFORCED blind Rating mode, so get-node never shows other raters' ratings/comments. For each node: read it once with get-node (claim + structure only), form your honest Agreement judgment FROM YOUR OWN PERSPECTIVE, and rate it:
  python3 graph.py get-node <nid> --data eggs-p10 --json
  python3 graph.py rate --agent ${a.id} --target <nid> --dim A --value <0.0-5.0> --bloc b1 --data eggs-p10 --json
...
phase('Rate')
log(`eggs-p10: ${agents.length} raters (2x2 disposition x model) rating all 79 nodes blind`)
const res = await parallel(agents.map(a => () =>
  agent(prompt(a), {
    label: `p10:${a.id}`, phase: 'Rate',
