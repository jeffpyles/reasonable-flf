#!/usr/bin/env python3
"""Emit one persona brief per agent (personas/{id}.md) from roster.json.

Each brief = who you are (perspective/bias/care/style) + the compact,
load-bearing rating rules (so an agent needs only this file + its assignment
+ the round instructions) + whether this agent is a BUILDER (may author to
extend the graph from source material).
"""
import json
...
def main():
    ...
    outdir = HERE / "personas"
    outdir.mkdir(exist_ok=True)
    # builders: all 8 experts + every high-care crowd member
    for a in roster["agents"]:
        is_builder = (a["tier"] == "expert") or (a.get("care") == "high")
        md = [f"# {a['id']}", "", a["brief"], ""]
        md.append(BUILDER_NOTE if is_builder else NON_BUILDER_NOTE)
        md.append("")
        md.append(RULES)
        (outdir / f"{a['id']}.md").write_text("\n".join(md))
    builders = [a["id"] for a in roster["agents"]
                if a["tier"] == "expert" or a.get("care") == "high"]
    print(f"wrote {len(roster['agents'])} persona briefs; {len(builders)} builders")
