#!/usr/bin/env python3
"""eggs-p5 persona briefs: same 60-agent roster as eggs-p4, but the task is a
single RATE-AND-PREDICT pass (Bayesian Truth Serum), not graph-building. Each
brief = who you are + the rating rules + the meta-prediction instruction.
"""
import json
...
def main():
    ...
    for a in roster["agents"]:
        md = [f"# {a['id']}", "", a["brief"], "", RULES, "", BTS, "", CLI]
        (outdir / f"{a['id']}.md").write_text("\n".join(md))
    print(f"wrote {len(roster['agents'])} eggs-p5 persona briefs (rate + predict)")
