#!/usr/bin/env python3
"""eggs-p9 analysis — de-confound DISPOSITION from MODEL, and validate the
...
Run: python3 eggs-p9/harness/analyze_p8.py    (from repo root, after the run)
"""
import json
import math
import random
...
roster = load_roster("eggs-p9")
disp = {a: d["disposition"] for a, d in roster.items()}
modl = {a: d["model_tier"] for a, d in roster.items()}

items = node_a_matrix("eggs-p9", restrict_ids=set(roster))
if not items:
    print("No eggs-p9 ratings yet — run workflow_p8.js first.")
    sys.exit(0)
...
for d in ("mild_biased", "neutral"):
    hs = cell[(d, "haiku")]; ss = cell[(d, "sonnet")]
    rowm = mean(hs + ss)
    print(f"{d:<10}{mean(hs):>10.2f}{mean(ss):>10.2f}{rowm:>10.2f}")
colh = mean(cell[("mild_biased", "haiku")] + cell[("neutral", "haiku")])
cols = mean(cell[("mild_biased", "sonnet")] + cell[("neutral", "sonnet")])
print(f"{'col mean':<10}{colh:>10.2f}{cols:>10.2f}")
disp_eff = (mean(cell[("neutral", "haiku")] + cell[("neutral", "sonnet")])
            - mean(cell[("mild_biased", "haiku")] + cell[("mild_biased", "sonnet")]))
mod_eff = cols - colh
inter = ((mean(cell[("neutral", "sonnet")]) - mean(cell[("mild_biased", "sonnet")]))
         - (mean(cell[("neutral", "haiku")]) - mean(cell[("mild_biased", "haiku")])))
...
print(f"  biased_sonnet accuracy {mean(cell[('mild_biased','sonnet')]):.2f} vs "
      f"neutral_sonnet {mean(cell[('neutral','sonnet')]):.2f} — "
      f"{'capability does NOT erase the prompted bias' if mean(cell[('mild_biased','sonnet')]) < mean(cell[('neutral','sonnet')]) - 0.1 else 'bias mostly absent on sonnet'}")
...
for label, anchors, at in ([("p5-panel (10)", p5_anchors, p5_at)] +
    ...
    biased_only = calibrate([a for a in raters if disp[a] == "mild_biased"], anchors, at)
...
print("\nDone. See eggs-p9/FINDINGS.md for the synthesis.")
